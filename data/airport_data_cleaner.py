...
def download_airport_data() -> None:
    ...
    if os.path.exists('airport_data.csv'):
    ...
    with open('airport_data.csv', 'wb') as f:
        f.write(r.content)

    print('Step 1 finished.')


def airport_class_file(airport_data: str, unique_flights: str) -> None:
    """Construct the file required for the creation of the Airport class"""
    if os.path.exists('airport_class.csv'):
    ...
    departures = pl.scan_parquet(source=unique_flights).select('segmentsDepartureAirportCode').unique().collect()
    arrivals = pl.scan_parquet(source=unique_flights).select('segmentsArrivalAirportCode').unique().collect()
    airports = (
        pl.scan_csv(source=airport_data)
    ...
    airports.collect().write_csv('airport_class.csv')
